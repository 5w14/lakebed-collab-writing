...
import { cleanAccess, cleanContent, cleanCoordinate, cleanRole, cleanTitle, cleanUserId } from "../shared/writing";
...
export default capsule({
  ...
  mutations: {
    ...
    inviteMember: mutation((ctx, documentId: string, userId: string, role: string) => {
      ...
      if (existing) {
        ctx.db.documentMembers.update(existing.id, { role: cleanRole(role), displayName: cleanId });
      } else {
        ctx.db.documentMembers.insert({ documentId, userId: cleanId, displayName: cleanId, role: cleanRole(role) });
      }
    }),
